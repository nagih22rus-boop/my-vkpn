...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "vkpn");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "vkpn");
  }
  ...
  FlMethodChannel* channel = fl_method_channel_new(
      messenger, "unified_vpn/methods", FL_METHOD_CODEC(codec));

  g_object_unref(codec);
  g_object_set_data_full(
      G_OBJECT(application), "unified_vpn_channel", channel,
  ...
  fl_method_channel_set_method_call_handler(
      ...
      [](FlMethodChannel* channel, FlMethodCall* call, gpointer user_data) {
        ...
        if (g_strcmp0(method, "start") == 0) {
          g_free(self->vpn_status);
          self->vpn_status = g_strdup("connected");
          g_autoptr(FlMethodResponse) response = FL_METHOD_RESPONSE(fl_method_success_response_new(nullptr));
          fl_method_call_respond(call, response, nullptr);
        } else if (g_strcmp0(method, "stop") == 0) {
          g_free(self->vpn_status);
          self->vpn_status = g_strdup("disconnected");
          g_autoptr(FlMethodResponse) response = FL_METHOD_RESPONSE(fl_method_success_response_new(nullptr));
          fl_method_call_respond(call, response, nullptr);
        } else if (g_strcmp0(method, "status") == 0) {
          g_autoptr(FlValue) value = fl_value_new_string(self->vpn_status == nullptr ? "disconnected" : self->vpn_status);
...
static void my_application_dispose(GObject* object) {
  ...
  g_clear_pointer(&self->vpn_status, g_free);
